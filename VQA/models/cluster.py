...
class FCM(nn.Module):
    """
    Initialize the frame-level compression module (FCM).
    Args:
...
class TSAttention(nn.Module):
    ...
    def forward(self, q_dict, kv_dict):
        ...
        q = q_dict['x']
        q = q.expand(-1, Nkv, -1)
        q = self.q(q).reshape(B, Nkv, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
...
class Att_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = TSAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward(self, v_token_dict, t_token_dict):

        kv_dict, _ = v_token_dict
        q_dict = t_token_dict
