...
from utils.metrics_qa import accuracy, AverageMeter
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0, tokenizer=ClipTokenizer()):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        input_ids, input_mask, video, video_mask, labels = batch

        ce_loss = model(input_ids, input_mask, video, video_mask, labels)
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    with torch.no_grad():
        for bid, batch in enumerate(test_dataloader):
            ...
            input_ids, input_mask, video, video_mask, labels = batch
            output = model(input_ids, input_mask, video, video_mask, labels)

            prec1, prec5, prec10 = accuracy(output, labels, topk=(1, 5, 10))
            top1.update(prec1[0], input_ids.size(0))
            top5.update(prec5[0], input_ids.size(0))
            top10.update(prec10[0], input_ids.size(0))

            print("{}/{}\r".format(bid, len(test_dataloader)), end="")

    logger.info("Video QA:")
    logger.info('>>> Prec@1: {top1.avg:.3f} - Prec@5: {top5.avg:.3f} - Prec@10: {top10.avg:.3f}'.
