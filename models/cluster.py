...
class Att_FCM(nn.Module):
    ...
    def forward(self, q_dict, kv_dict):
        A, Nq, C = q_dict['x'].shape
        B, Nkv, C = kv_dict['x'].shape
        kv = kv_dict['x']
        conf_kv = kv_dict['token_score'] if 'token_score' in kv_dict.keys() else kv.new_zeros(B, Nkv, 1)

        q = q_dict['x']
        q = q.expand(-1, Nkv, -1)
        q = self.q(q).reshape(A, Nkv, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
        ...
        attn = (k * self.scale) @ q.transpose(-2, -1)
        conf_kv = conf_kv.squeeze(-1)[:, None, None, :]
        attn = attn + conf_kv
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, Nkv, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x

class Att_Block_Frame(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Att_FCM(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
...
if __name__ == '__main__':
    ...
    v_fcm_f = FCM(sample_ratio=sample_ratio, embed_dim=embed_dim, dim_out=embed_dim, k=3)
    v_att_block_f = Att_Block_Frame(dim=embed_dim, num_heads=8)
    v_pcm_p = PCM(sample_ratio=sample_ratio, embed_dim=embed_dim, dim_out=embed_dim, k=3)
    v_att_block_p = Att_Block_Patch(dim=embed_dim, num_heads=8)

    a, b, f, p, d = 1, 32, 12, 50, 512
    p_feat = torch.rand(b, f * p, d)
    ...
    s_token_dict = {'x': s_feat,
                    ...
                    'mask': s_mask.detach()}
    p_token_dict = v_att_block_f(s_token_dict, v_fcm_f(p_token_dict))
    p_feat = p_token_dict['x']
    p_mask = p_token_dict['mask']
