...
Key Functions:
  ...
  - PCM (Progressive Clustering Module): Applies TokenConv + LayerNorm, predicts
    per-token importance scores, then calls DPC-KNN for downsampling.
  - Cross_Att_PCM: Cross-attention with mismatched query/key-value dimensions.
    ...
    receive an optional token_score confidence bias before softmax.
  - Cross_Att_Block_Patch: Transformer block wrapping Cross_Att_PCM. Applies
    ...
    the key/value count, and the attention output is added to kv_dict.x via a
    residual connection. The returned dict is therefore the updated kv_dict.
...
def merge_tokens(token_dict, idx_cluster, cluster_num, token_weight=None):
    ...
    Returns:
        dict: output token information dict with merged tokens, containing:
            ...
            - agg_weight (Tensor): updated aggregation weights (original * per-token norm_weight).
    ...
    # Normalize token weights per cluster. Use clamp(min=1e-4) instead of +1e-6
    # because index_add_ may leave some clusters with near-zero total weight
    # when tokens are heavily masked or sample_ratio is very small; a stronger
    # lower bound avoids extreme norm_weight values that explode into NaN.
    all_weight = all_weight.clamp(min=1e-4)
    ...
    # Update agg_weight: multiply original aggregation weight by the per-token normalization weight
    weight_t = index_points(norm_weight, idx_token)
    agg_weight_new = agg_weight * weight_t

    # If mask exists, sum within each cluster; a cluster is valid if its total mask sum > 0
...
class PCM(nn.Module):
    ...
    PCM is the core token compression module. It performs the following steps:
        ...
        3. Convert scores to non-negative aggregation weights via exponential.
    ...
    def forward(self, token_dict):
        ...
        # Step 6: Merge tokens by cluster to get downsampled token_dict
...
class Cross_Att_PCM(nn.Module):
    ...
    bias on the attention logits rather than a gating term. Invalid key/value
    positions can be masked out via kv_dict['mask'] (0 = invalid).
    ...
    def forward(self, q_dict, kv_dict):
        ...
        Args:
            q_dict (dict): token dict for queries. Must contain 'x'.
                If Nq == 1, the query will be broadcast to match Nkv.
            kv_dict (dict): token dict for keys and values. Must contain 'x'.
                Optionally contains 'token_score' (used as attention bias) and
...
class Cross_Att_Block_Patch(nn.Module):
    ...
    cross-attention, and adds the attended output back to kv_dict['x'] via a
    residual connection. The returned dict is therefore the updated kv_dict.
    ...
    def forward(self, q_dict, kv_dict):
        ...
        # Residual branch uses the original kv features, projected if dims mismatch
        res = self.proj_res(kv_x)

        # Cross-attention: queries attend over keys/values.
        # If Nq == 1 the query is broadcast so output shape matches Nkv.
        x = res + self.drop_path(self.attn(q_dict, kv_dict))
...
if __name__ == '__main__':
    ...
    # Text sentence tokens: [batch_size, 1, embed_size]
    t_feat = torch.rand(batch_size, 1, embed_size)
    t_idx_token = torch.arange(t_feat.size(1))[None, :].repeat(t_feat.size(0), 1)
    t_agg_weight = t_feat.new_ones(t_feat.size(0), t_feat.size(1), 1)
    t_mask = t_feat.new_ones(t_feat.size(0), t_feat.size(1))
    t_token_dict = {'x': t_feat,
                    'token_num': t_feat.size(1),
                    'idx_token': t_idx_token,
                    'agg_weight': t_agg_weight,
                    'mask': t_mask.detach()}
    print("Cross0--->>>", p_token_dict['x'].shape)
    p_token_dict = v_cross_att_block_p_1(t_token_dict, v_pcm_p_1(p_token_dict))
    print("Cross1--->>>", p_token_dict['x'].shape)
    p_token_dict = v_cross_att_block_p_2(t_token_dict, v_pcm_p_2(p_token_dict))
    print("Cross2--->>>", p_token_dict['x'].shape)
    p_token_dict = v_cross_att_block_p_3(t_token_dict, v_pcm_p_3(p_token_dict))
